...
of the provenance of this package's products ('1A-*', '1B-*'); write to file.
...
Pypackages_to_query = ("PREFIRE_L1", "PREFIRE_PRD_GEN", "PREFIRE_tools")

product_monikers = {3:"1A-RAD", 4:"1B-RAD", 103:"1A-GEOM", 104:"1B-GEOM"}


#--------------------------------------------------------------------------
def main(anchor_path, proc_mode, geom_only=False):
    """Driver routine."""
    ...
    if geom_only:
        product_moniker = product_monikers[proc_mode+100]
    else:
...
if __name__ == "__main__":
    ...
    arg_description = ("Determine product monikers and current (latest) git "
                       ...
                       "package's products ('1A-*', '1B-*'); write to file.")
    arg_parser = argparse.ArgumentParser(description=arg_description)
    arg_parser.add_argument("--geom", action="store_true")
    ...
    # Run driver:
    main(anchor_path, proc_mode, args.geom)
